...
enum AgentDetector {
    ...
    private static func detectCopilotCLI() -> AgentDetection {
        ...
        // Probe 6: DoomCode global hooks file is the strongest "configured" signal.
        let hooksPath = NSHomeDirectory() + "/.copilot/hooks/doomcoder.json"
        if FileManager.default.fileExists(atPath: hooksPath) {
            return AgentDetection(agent: .copilotCLI, installed: true, version: nil, details: "DoomCode hooks installed")
        }
