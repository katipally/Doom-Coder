    ...
    private func hooksSetupSection(_ agent: TrackedAgent) -> some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 8) {
                Text("Installs hooks so Doom Coder can see when \(agent.displayName) starts, finishes, or needs your approval. Hooks run locally on this Mac — your code and prompts are never sent to DoomCoder.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
